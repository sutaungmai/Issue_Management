import { useEffect, useState,moment } from "react"
import { request } from "../network/networkRequest"
import { DATETIME_FORMAT} from "../config/config"
    ...
    const load = () => {
        ...
        request(
            ...
            setData(res.data.map(x=> ({
                ...x,
                start_date: moment(x.start_date).format(DATETIME_FORMAT),
                close_date: moment(x.close_data).format(DATETIME_FORMAT),
            })))
            setError({})
        }).catch(err=>{
            setSuccess(false)
